...
export default function Home(props: HomeProps) {
  ...
  return (
    <div className="max-w-[1124px] h-screen mx-auto grid grid-cols-2 gap-28 items-center">
      <main>
        ...
        <form onSubmit={createPool} className="mt-10 flex gap-2 text-gray-100">
          <input
            ...
            onChange={even => setPoolTitle(even.target.value)}
            value={poolTitle}
          />
          <button
      ...
      <Image
        src={appPreviewImage} alt="Dois celulares exibindo uma prévia da aplicação NLW"
        quality={100}
      />
    </div>
  )
...
export const getServerSideProps = async () => {
  ...
  const [poolCount, guessCountResponse, userCountResponse] = await Promise.all([
    api.get('pools/count'),
